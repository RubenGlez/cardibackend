...
export default class CheckBusinessAuthenticationUseCase {
  ...
  async run(accessToken?: Auth['accessToken']): Promise<User['id']> {
    if (accessToken === undefined) throw new OutputError(OutputErrorTypes.MissingAccessToken)
    ...
    if (user.role !== UserRole.Business) throw new OutputError(OutputErrorTypes.InvalidUserRole)
